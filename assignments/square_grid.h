...
public:
  square_grid() : width(0) {}
  square_grid(int width_) : width(width_) {}
...
template <class T>
grid_location<float> square_grid<T>::pixel_to_tile(float size, int x, int y)
{
  auto px = (x / size);
  auto py = (y / size);
...
template <class T>
grid_location<int> square_grid<T>::tile_round(grid_location<float> t)
{
