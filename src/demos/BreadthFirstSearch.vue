...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      bfsResults() {
        const ref = wasmModule.BreadthFirstSearch(
          this.gridWorld,
          this.goalToGrid,
          this.stepLimit
        );
        return {
          frontier: Array.from({ length: ref.first.size() }, (_, i) =>
            ref.first.get(i)
          ),
          came_frome: ref.second,
        };
      },
      centerPosition: {
        get() {
    ...
    methods: {
      ...
      classFor(location) {
        ...
        return this.gridWorld.isWall(location)
          ? 'wall'
          : bfsResult.frontier.some(
                (item) => item.x == location.x && item.y == location.y
              )
            ? 'frontier'
            : bfsResult.came_frome.get(location)
              ? 'explored'
              : '';
      },
    },
